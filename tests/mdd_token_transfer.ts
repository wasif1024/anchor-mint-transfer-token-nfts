...
import { MddTokenTransfer } from "../target/types/mdd_token_transfer";

describe("mdd_token_transfer", () => {
  // Configure the client to use the local cluster.
  // Configure the client to use the local cluster.
  anchor.setProvider(anchor.AnchorProvider.env());
  // Retrieve the TokenContract struct from our smart contract
  const program = anchor.workspace.MddTokenTransfer as Program<MddTokenTransfer>;
  ...
  it("Mint a token", async () => {
    ...
    //MINT_SIZE
    //);
    const lamports: number = await anchor.getProvider().connection.getMinimumBalanceForRentExemption(
      MINT_SIZE
    );
    ...
    console.log(
      await anchor.getProvider().connection.getParsedAccountInfo(mintKey.publicKey)
    );
    ...
    /*await program.state.rpc.mint_token({
      accounts: {
        authority: provider.wallet.publicKey,
      },
    });*/
    // Get minted token amount on the ATA for our anchor wallet
    const minted = (await anchor.getProvider().connection.getParsedAccountInfo(associatedTokenAccount)).value.data.parsed.info.tokenAmount.amount;
